/**
 * ========================================
 * CAMINHO: /utmtrack/assets/js/adsets-dashboard.js
 * ========================================
 * 
 * UTMTrack - JavaScript Dashboard de Conjuntos
...
const allColumns = {
    'nome': { label: 'Nome do Conjunto', default: true, minWidth: 250, sortable: true, editable: true },
    'status': { label: 'Status', default: true, minWidth: 100, sortable: true, editable: true },
    'orcamento': { label: 'Orçamento Diário', default: true, minWidth: 130, sortable: true, editable: true },
...
function makeEditableName(element, adsetId) {
    if (element.classList.contains('editing')) return;
    
    const row = element.closest('tr');
    const metaAdSetId = row ? row.getAttribute('data-adset-id') : '';
    ...
    element.innerHTML = `<input 
        type="text" 
        value="${currentValue}" 
        onblur="saveField(this, ${adsetId}, 'adset_name', 'text', '${metaAdSetId}')"
...
function makeEditable(element, adsetId, field, type = 'text', metaAdSetId = '') {
    if (element.classList.contains('editing')) return;
    ...
    element.innerHTML = `<input 
        ...
        ${stepAttr}
        ${minAttr}
        onblur="saveField(this, ${adsetId}, '${field}', '${type}', '${metaAdSetId}')"
        onkeypress="if(event.key==='Enter') this.blur()"
...
async function saveField(input, adsetId, field, type, metaAdSetId = '') {
    ...
    try {
        ...
        const response = await fetch(
            `${window.location.origin}/utmtrack/index.php?page=conjuntos&ajax_action=${action}`, 
            {
                method: 'POST',
...
async function toggleAdSetStatus(checkbox, adsetId, metaAdSetId) {
    ...
    try {
        const response = await fetch(
            window.location.origin + '/utmtrack/index.php?page=conjuntos&ajax_action=update_status', 
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
...
async function syncAllAdSets() {
    ...
    try {
        const response = await fetch(
            window.location.origin + '/utmtrack/index.php?page=conjuntos&ajax_action=sync_all',
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'same-origin'
...
function filterTable() {
    ...
    console.log(`🔍 Filtros aplicados: ${visibleCount} conjuntos visíveis`);
}
...
async function saveColumns() {
    try {
        ...
        const response = await fetch(
            window.location.origin + '/utmtrack/index.php?page=conjuntos&ajax_action=save_columns', 
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function saveColumnWidths() {
    localStorage.setItem('utmtrack_adsetColumnWidths', JSON.stringify(columnWidths));
}

function loadColumnWidths() {
    const saved = localStorage.getItem('utmtrack_adsetColumnWidths');
    if (saved) {
        try {
...
function saveColumnsOrder() {
    localStorage.setItem('utmtrack_adsetColumnsOrder', JSON.stringify(selectedColumnsList));
}

function loadColumnsOrder() {
    const saved = localStorage.getItem('utmtrack_adsetColumnsOrder');
    if (saved) {
        try {
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Dashboard Conjuntos v14.1 carregado');
...
// Exporta para window
window.toggleAdSetStatus = toggleAdSetStatus;
window.makeEditableName = makeEditableName;
window.makeEditable = makeEditable;
window.saveField = saveField;
window.syncAllAdSets = syncAllAdSets;
